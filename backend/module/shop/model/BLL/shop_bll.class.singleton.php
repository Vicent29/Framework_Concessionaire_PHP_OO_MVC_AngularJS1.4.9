...
class shop_bll
{
	...
	public function get_shop_filters_BLL($args){
		...
		return $this->dao->select_shop_filters($this->db, $filtros);
	}
	...
	public function get_home_filter_BLL($args){
		...
		return $this->dao->select_filter_home($this->db, $filter);
	}
	...
	public function get_search_filter_BLL($args){
		...
		$city = $args[2];
			...
			if (($motor != "0") && ($brand == "0") && ($city == "0")) {
				return $this->dao->select_motor_search($this->db, $motor);
			} else if (($motor == "0") && ($brand != "0") && ($city == "0")) {
				return $this->dao->select_brand_search($this->db, $brand);
			} else if (($motor == "0") && ($brand == "0") && ($city != "0")) {
				return $this->dao->select_city_search($this->db, $city);
			} else if (($motor != "0") && ($brand != "0") && ($city == "0")) {
				return $this->dao->select_motor_brand_search($this->db, $motor, $brand);
			} else if (($motor == "0") && ($brand != "0") && ($city != "0")) {
				return $this->dao->select_brand_city_search($this->db, $brand, $city);
			} else if (($motor != "0") && ($brand == "0") && ($city != "0")) {
				return $this->dao->select_motor_city_search($this->db, $motor, $city);
			} else if (($motor != "0") && ($brand != "0") && ($city != "0")) {
				return $this->dao->select_all_search($this->db, $motor, $brand, $city);
			} else {
				return $this->dao->select_all_cars($this->db);
			}
	}
	...
	public function get_order_filter_BLL($args){
		...
		if ($args[0] == "") {
			...
			$Dates_Cars = $this->dao->select_all_cars_order($this->db, $args[0]);
		}
		return $Dates_Cars;
	}
